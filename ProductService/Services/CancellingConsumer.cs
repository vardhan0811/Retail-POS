...
{
    /// <summary>
    /// Dedicated consumer for bill.cancelled events.
    /// Releases (unreserves) stock for the affected store — does NOT deduct physical stock.
    /// </summary>
    public sealed class CancellingConsumer : Shared.Messaging.RabbitMq.RabbitMqConsumerBase
    {
        private readonly IServiceScopeFactory _scopeFactory;

        public CancellingConsumer(
            IConfiguration config,
            ILogger<CancellingConsumer> logger,
            IServiceScopeFactory scopeFactory)
            : base(config, logger, queueName: "product.bill.cancelled", routingKeys: "bill.cancelled")
        {
        ...
        protected override async Task ProcessMessageAsync(byte[] body, string routingKey, CancellationToken ct)
        {
            ...
            var e = JsonSerializer.Deserialize<BillCancelledEvent>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true })

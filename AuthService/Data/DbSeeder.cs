...
namespace AuthService.Data
{
    public static class DbSeeder
    {
        ...
        private static async Task EnsureCashierCanUpdateProductAsync(AuthDbContext context)
        {
            ...
            var updateProduct = await context.Permissions.FirstOrDefaultAsync(p => p.Name == "UPDATE_PRODUCT");

            if (cashierRole == null || updateProduct == null)
                return;

            var mappingExists = await context.RolePermissions.AnyAsync(rp =>
                rp.RoleId == cashierRole.Id && rp.PermissionId == updateProduct.Id);
